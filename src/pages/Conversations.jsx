...
// //       // 2. FETCH HISTORICAL MESSAGES (Matched to your backend: /chat/messages/{id})
...
// //         setMessages((prev) => [...prev, { ...data, text: plaintext }]);
// //       };
// //     } catch (err) {
...
// //     e.preventDefault();
// //     if (!input.trim() || !ws.current || !friendBundle) return;

// //     const encryptedData = await CryptoService.encryptMessage(input);
// //     const payload = {
...
// //     setMessages(prev => [...prev, { sender_id: user.id, text: input, created_at: new Date().toISOString() }]);
// //     setInput('');
// //   };
...
// //           {friends.length === 0 && <p className="text-zinc-600 text-center py-10 text-sm italic">No contacts found</p>}
// //           {friends.map(friend => (
// //             <button
// //               key={friend.id}
...
// //                   ? 'bg-indigo-600/10 border-indigo-500/30 text-indigo-400 shadow-[0_0_15px_rgba(79,70,229,0.1)]' 
// //                   : 'hover:bg-zinc-800/50 border-transparent text-zinc-400'
// //               }`}
// //             >
// //               <div className="w-12 h-12 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center font-bold text-lg text-zinc-100">
...
// //                     </span>
// //                   </div>
// //                   <p className="text-xs text-zinc-500 truncate max-w-md mt-1">
// //                     {activeChat.bio || "Secure communication is a human right."}
// //                   </p>
// //                 </div>
// //               </div>
// //               <div className="flex items-center gap-3">
// //                 <button className="p-2 text-zinc-500 hover:text-white transition-colors"><Info size={20}/></button>
// //                 <button className="p-2 text-zinc-500 hover:text-white transition-colors"><MoreVertical size={20}/></button>
// //               </div>
// //             </header>

// //             <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-4 scrollbar-hide relative">
// //               {isLoadingHistory ? (
...
// //                   {messages.length === 0 && (
// //                     <div className="text-center py-20 text-zinc-600 text-sm italic opacity-50">No messages yet. Send a secure greeting!</div>
// //                   )}
// //                   {messages.map((msg, i) => {
...
//   // Handles 'Z' suffix from backend to show your laptop's local time
...
//       ws.current.onopen = () => console.log("🚀 WebSocket Connected to conversation:", conversationId);
...
//     // 🛡️ Safety Check: Ensure WebSocket is fully connected before sending
...
//       // Update UI immediately with local time
